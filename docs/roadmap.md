   ...
   48. (✅️finished) 訓練强化學習模型操控 Unitree G1 行走 
   ...
   54. 製作一個帶有明確規劃的文檔，清晰説明不同區塊/組件/功能的職責
        - object_template/[object name]/models 裏面添加 obs 不同位置數據的文字説明映射，和 nan report 搭配 

   55. 添加一種新的獎勵類型：基於參考軌跡的逐幀跟踪獎勵（Reference Trajectory Tracking Reward）：在策略執行期間，每一時間步根據當前關節角度與角速度與預錄製的標準揮拳軌跡的偏差，計算指數級匹配獎勵，引導强化學習策略復現目標動作。
