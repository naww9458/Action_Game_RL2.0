...
# Resume from an existing RSL-rl checkpoint:
  python train.py --resume runs/RSL-rl/<run>/checkpoints/model_500.pt

Output
──────
Checkpoints, TensorBoard logs and the preset config snapshot are written to:
  runs/RSL-rl/<YY-MM-DD_HH-MM-SS-ffffff_RSL_RL_PPO_flat_walk>/

Monitor training with:
  tensorboard --logdir runs/RSL-rl
"""
...
class TrainArgs:
    ...
    preset: str = "flat_walk_rsl_rl_ppo_state_based"
    num_envs: int = 4096
    device: str = "cuda:0"
...
def run_train(args: TrainArgs) -> None:
    ...
    print(
        f"[INFO] Preset '{args.preset}' | env={loaded.meta.env_id} "
    ...
    # Create the log directory under runs/RSL-rl/.
    run_parent = framework_runs_dir(loaded.meta.framework)
    run_parent.mkdir(parents=True, exist_ok=True)
    log_dir = run_parent / make_experiment_name(
        loaded.meta.env_id,
        loaded.meta.algorithm,
        framework=loaded.meta.framework,
