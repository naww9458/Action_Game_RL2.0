...
def _environment_cfg_path(config_path: Path) -> Path:
    current = config_path / "environment_cfg.yaml"
    if current.exists():
        return current
    return config_path / "level_cfg.yaml"
...
class Trainer_base:
    ...
    def save_config_pickle(self, model_cfg, train_cfg, environment_cfg, path):
        path = str(path)
        ...
        with open(path + "/environment_cfg.yaml", "w", encoding="utf-8") as f:
            yaml.dump(environment_cfg, f, default_flow_style=False, sort_keys=False)

    def save_run_config(self, config_path, environment_cfg):
    ...
    def load_config_from_checkpoint(self, path):
        ...
        if preset_yaml.exists():
            ...
            env_cfg_path = _environment_cfg_path(config_path)
            return loaded.model_cfg, loaded.train_cfg, str(env_cfg_path), loaded
        ...
        return model_cfg, train_cfg, str(env_cfg_path), loaded

    def load_config_pickle(self, path):
        model_cfg, train_cfg, env_cfg_path, _ = self.load_config_from_checkpoint(path)
        return model_cfg, train_cfg, env_cfg_path
