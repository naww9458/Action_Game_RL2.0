...
class RewardCalculator:
    ...
    def __init__(self, 
        ...
        # ``get_metric_means``) 對所有 level / 框架通用。
    ...
    def get_reward_terms(self) -> Dict[str, torch.Tensor]:
        ...
        The returned value is scaled by the level's ``reward_log_scale`` (1.0 by
        default; Level5_0 uses FPS=50) so logged ``Episode_Reward/*`` match
        mjlab's per-second units (per-step mean x FPS = sum/20). The diagnostic
        accumulator keeps the raw per-step mean.
        """
        scale = 1.0
        if self.level is not None:
            scale = float(getattr(self.level, "reward_log_scale", 1.0))
