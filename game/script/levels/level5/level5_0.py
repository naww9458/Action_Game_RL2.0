...
class Level5_0(Levels):
    ...
    def setup(self):
        ...
        obs_noise_cfg = None
        env_configs = self.level_configs.get("environment_configs") or {}
        dr_cfg = {}
        if isinstance(env_configs, dict):
            obs_noise_cfg = env_configs.get("observation_noise")
            dr_cfg = env_configs.get("domain_randomization") or {}

        # C4: encoder_bias / base_com DR (mjlab startup events). Bias corrupts
        # the actor observation; base_com is an obs-domain approximation of the
        # torso COM offset (the shared mujoco-warp model cannot hold per-env
        # body_ipos). ``None`` disables the term.
        encoder_bias_range = None
        enc_cfg = dr_cfg.get("encoder_bias") or {}
        if enc_cfg.get("enabled", True):
            br = enc_cfg.get("bias_range") or [-0.015, 0.015]
            encoder_bias_range = (float(br[0]), float(br[1]))
        base_com_offset_range = None
        com_cfg = dr_cfg.get("base_com") or {}
        if com_cfg.get("enabled", True):
            cr = com_cfg.get("offset_range") or {}
            base_com_offset_range = {
                "x": (float(cr.get("x", [-0.025, 0.025])[0]), float(cr.get("x", [-0.025, 0.025])[1])),
                "y": (float(cr.get("y", [-0.025, 0.025])[0]), float(cr.get("y", [-0.025, 0.025])[1])),
                "z": (float(cr.get("z", [-0.03, 0.03])[0]), float(cr.get("z", [-0.03, 0.03])[1])),
            }

        self.g1_provider = self._create_registered_obs_provider(
            pattern,
            obs_noise_cfg=obs_noise_cfg,
            encoder_bias_range=encoder_bias_range,
            base_com_offset_range=base_com_offset_range,
        )
        self._wire_encoder_bias_into_action()
        ...
        # missing, the level gracefully falls back to the symmetric critic.
        ...
        if self.critic_obs_provider is not None:
            print(
                f"[Level5_0] asymmetric critic extension active: "
    ...
    def _create_registered_obs_provider(
        self,
        pattern: str,
        *,
        obs_noise_cfg,
        ...
        """Resolve the observation provider from the player's control-policy bundle.

        The factory is registered by the robot's ``object_template/*/register.py``
        under ``observation.provider`` in ``models/<version>/control_policy.yaml``.
        The level never imports a concrete provider subclass.
        """
        ...
        object_cfg = dict(player_configs[0].get("object") or {})
        version = object_cfg.get("control_policy_version")
        if not version:
            raise ValueError(
                "player_configs[0].object.control_policy_version is required to "
                "resolve the observation provider."
            )
        bundle = PolicyBundleRegistry.get(str(version), robot_pattern=pattern)
        return PolicyBundleRegistry.create_obs_provider(
            bundle.obs_provider,
    ...
    def _wire_encoder_bias_into_action(self):
        """Share the provider's encoder-bias buffer with the PD action applier.

        mjlab subtracts the same bias from actor joint_pos *and* from the PD
        target (``target = default + scale*a - bias``). The ability starts with
        a zeros buffer; replace it with the provider's when present. Missing
        setter / zeros bias is a no-op (play / rl_assisted inference).
        """
        bias = getattr(self.g1_provider, "encoder_bias_wp", None)
        if bias is None:
            return
    ...
    def _get_critic_observation(self) -> torch.Tensor:
        ...
        not ready), the level falls back to the symmetric critic (policy obs).
        """
