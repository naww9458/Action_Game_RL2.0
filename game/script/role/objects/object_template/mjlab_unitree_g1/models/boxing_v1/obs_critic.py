...
class G1FootCriticObs:
    ...
    def try_describe_obs_index(self, index: int) -> str | None:
        """Layout label for critic extras (policy columns are labeled by the actor)."""
        i = int(index)
        policy_dim = int(self.policy_obs_dim)
        if i < policy_dim:
            return None
        extra = i - policy_dim
        n = int(self.num_feet)
        if extra < n:
            return f"critic.foot_height[{extra}]"
        extra -= n
        if extra < n:
            return f"critic.foot_air_time[{extra}]"
        extra -= n
        if extra < n:
            return f"critic.foot_contact[{extra}]"
        extra -= n
        foot = extra // 3
        axis = extra % 3
        axis_name = "xyz"[axis] if axis < 3 else str(axis)
        return f"critic.foot_force[{foot}].{axis_name}"
