...
def _attach_punch_phase(environment: Any, bundle: PolicyBundleSpec, obs_actor: Any) -> None:
    """Create the punch/retract hard-state FSM and bind its signal into the actor obs."""
    mod = PolicyBundleRegistry.import_version_module(bundle, "punch_phase")
    if mod is None:
        return
    factory = getattr(mod, "create_punch_phase", None)
    if not callable(factory):
        return
    punch_phase = factory(
        environment=environment,
        obs_actor=obs_actor,
        num_env=int(environment.num_env),
        device=GameConfig.DEVICE,
        dt=1.0 / float(GameConfig.FPS_ACTION),
        seed=int(getattr(GameConfig, "SEED", 31415926)),
    )
    if not getattr(punch_phase, "_ready", False):
        return
    environment.punch_phase = punch_phase
    # Bind the FSM signal buffer into the actor's packed obs (action-signal col).
    signal_wp = getattr(punch_phase, "signal_wp", None)
    if signal_wp is not None and getattr(obs_actor, "signal_wp", None) is not None:
        obs_actor.signal_wp = signal_wp
    print("[G1PolicyRuntime] punch_phase FSM active (action-signal obs column)")
...
class G1PolicyRuntime:
    ...
    def on_reset(self, terminated, current_step) -> None:
        ...
        punch_phase = getattr(self._environment, "punch_phase", None)
        if punch_phase is not None:
            punch_phase.reset(terminated_int.bool())
    ...
    def on_update_game_status(self, physics_manager, reward_calculator, num_env, current_step) -> None:
        ...
        punch_phase = getattr(self._environment, "punch_phase", None)
        if punch_phase is not None:
            punch_phase.step(physics_manager)
...
def attach_if_present(environment: Any) -> Optional[G1PolicyRuntime]:
    ...
    _attach_punch_phase(environment, bundle, obs_actor)
