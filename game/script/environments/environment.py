...
class Environment:
    def __init__(self, game: 'Game', config: dict = None):
        ...
        self._object_template_runtime_hooks: List[Any] = []
        ...
        print("Setup Player!")
        self.players: Player = Player(configs=self.config.get("player_configs", []))
        print("Setup Platform!")
        self.platforms: Platform = Platform(configs=self.config.get("platform_configs", []))
        print("Setup Entity!")
        self.entities: Entity = Entity(configs=self.config.get("entity_configs", []))
        print("Setup Tool!")
        tool_configs = self.config.get("tool_configs") or []
        ...
        self.abilities_objects: AbilityGeneratedObject = AbilityGeneratedObject(configs=self.config.get("ability_generated_object_configs", []))
    ...
    def _inherit_host_spawn_pose_for_tools(self, tool_configs: List[dict]) -> None:
        ...
        """
        player_configs = self.config.get("player_configs") or []
        host_by_id = {}
    ...
    def resolve_player_pattern(self, player_index: int = 0) -> str:
        """Resolve articulation-body player pattern from environment YAML player_configs."""
        player_configs = self.config.get("player_configs", [])
        if not player_configs:
            raise ValueError("environment config is missing player_configs.")
    ...
    def setup(self):
        ...
        if self.mount_joint_registry is not None and self.mount_joint_registry.records:
            solver_type = str(
                (self.config.get("environment_configs") or {}).get("solver_config", {}).get(
                    "type", ""
                )
            )
        ...
        self._run_object_template_setups()

        return self.players, self.platforms, self.entities, self.abilities_objects

    def _run_object_template_setups(self) -> None:
        """Run object-template runtime hooks for objects actually in this environment.

        Mirrors reward_components: the base class only iterates; each template's
        ``register.setup(environment)`` no-ops when its object was not loaded.
        """
        run_object_template_setups(self)
    ...
    def apply_object_template_actions(self):
        """Run lazy object-template per-frame control writes (same phase as player action)."""
        for hook in getattr(self, "_object_template_runtime_hooks", None) or []:
            apply_fn = getattr(hook, "apply", None)
            if callable(apply_fn):
                apply_fn()

    def _dispatch_template_hooks(self, method_name: str, *args, **kwargs):
        """Call optional methods on object-template runtime hooks.

        Base classes stay unaware of concrete templates: missing methods are
        skipped. The last non-None return value is forwarded to the caller.
        """
        result = None
        for hook in getattr(self, "_object_template_runtime_hooks", None) or []:
    ...
    def action(self):
        ...
        """
        # Nothing to do in the base environment
        pass
    ...
    def update_game_status(self, physics_manager: 'PhysicsManager', reward_calculator: 'RewardCalculator', num_env: wp.int32, current_step: wp.array):
        self._dispatch_template_hooks(
            "on_update_game_status",
            physics_manager,
            reward_calculator,
            num_env,
            current_step,
    ...
    def on_step_actions(self, actions_wp):
        self._dispatch_template_hooks("on_step_actions", actions_wp)

    def _get_observation_state_based(self) -> 'torch.Tensor':
        """
        Return the current observation without taking a step
        """
        obs = self._dispatch_template_hooks("get_observation")
        if obs is not None:
            return obs
        return getattr(self, "obs_torch", None)

    def get_command_metrics(self) -> Optional[dict]:
        """Return per-step command-tracking metrics, or ``None``.

        Framework-agnostic public interface: environments that track a reference
        ...
        environment has no such metrics and the caller simply skips them.
        """
        return None
    ...
    def get_curriculum_metrics(self) -> Optional[dict]:
        ...
        Environments that sample commands from curriculum-style ranges override this to
        return ``dict[str, torch.Tensor]`` of 0-d GPU tensors (keys already carry
        the ``Curriculum/`` prefix). ``None`` means the environment has no curriculum
    ...
    def get_log_key_order(self) -> Optional[tuple[str, ...]]:
        ...
        Lets an environment align its training-log lines with a reference
        implementation (e.g. mjlab) that prints extras in a fixed, non-insertion
        order. Keys produced by the environment but absent from the template keep
        their natural insertion position at the end; template keys the environment
    ...
    def update_curriculum(self):
        ...
        Environment implementations (e.g. FlatWalk) advance a global step counter
        and schedule curriculum changes here. Called from host code outside the
    ...
    def _restore_start_attached_tools(self, terminated) -> None:
        """After a reset, re-attach tools whose environment config has `start_attached: true`.
...
class DefaultEnvironment(Environment):
    """Default environment used when no custom Python class is provided."""
    ...
    def _get_observation_state_based(self) -> torch.Tensor:
        obs = super()._get_observation_state_based()
        if obs is not None:
            return obs
