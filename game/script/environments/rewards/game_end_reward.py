...
class G1LocomotionTerminator(RewardComponent):
    ...
    def bind_environment(self, environment):
        if self.pattern is None and hasattr(environment, "resolve_player_pattern"):
            self.pattern = environment.resolve_player_pattern()
        if self.pattern is None:
            raise ValueError(
                "G1LocomotionTerminator requires `pattern` via constructor kwargs "
                "or environment.resolve_player_pattern()."
            )
        self._resolve_view()
